...
# Трасса: что за фон увидел Stop (в паре с ~/.claude/run/alerts.log от agent-alert.sh).
printf '%s STOP pane=%s cwd=%s alert=%s tasks=%s\n' "$(date +%H:%M:%S)" "$TMUX_PANE" \
  "$(basename "$(printf '%s' "$input" | jq -r '.cwd // ""' 2>/dev/null)")" "$alert" \
  "$(printf '%s' "$input" | jq -c '[.background_tasks[]? | {type,status}]' 2>/dev/null)" \
