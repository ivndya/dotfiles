...
# jq, а не grep -oP: в BSD-grep (macOS) нет PCRE, путь выходил пустым и
# watchdog крутился вхолостую (`[ -f "" ]` → continue навсегда).
...
# setsid есть в Linux/WSL, но НЕ в macOS — там nohup + фон (хук всё равно
# завершается сразу, процесс переживает возврат).
if command -v setsid >/dev/null 2>&1; then
  setsid "$HOME/.claude/hooks/watchdog.sh" "$transcript" "$TMUX_PANE" "$pidfile" \
    </dev/null >/dev/null 2>&1 &
else
  nohup "$HOME/.claude/hooks/watchdog.sh" "$transcript" "$TMUX_PANE" "$pidfile" \
    </dev/null >/dev/null 2>&1 &
  disown 2>/dev/null
fi
