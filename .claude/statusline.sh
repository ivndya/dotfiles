...
now_ts=$(date +%s)
# UNIX-время из "YYYY-MM-DD HH:MM:SS". BSD (macOS) и GNU (Linux/WSL) парсят
# датой по-разному: `date -j -f` против `date -d`. BSD пробуем первым — на маке
# `-d` вообще невалиден (там это флаг DST), и без этой ветки peak_*_ts выходили
# пустыми, а маркер 🔥 не показывался никогда.
epoch_at() {  # $1 = "YYYY-MM-DD HH:MM:SS", $2 = "utc" | ""
    if [ "$2" = "utc" ]; then
        date -u -j -f '%Y-%m-%d %H:%M:%S' "$1" +%s 2>/dev/null || date -u -d "$1" +%s 2>/dev/null
    else
...
if [ -n "$CLAUDE_STATUSLINE_PEAK_HOURS_LOCAL" ]; then
    ...
    peak_start_ts=$(epoch_at "${today_local} ${peak_start_h}:00:00")
    peak_end_ts=$(epoch_at "${today_local} ${peak_end_h}:00:00")
    dow=$(date +%u)
else
    # Anthropic default: 13:00-19:00 UTC будни (UTC date для согласованности)
    today_utc=$(date -u +%Y-%m-%d)
    peak_start_ts=$(epoch_at "${today_utc} 13:00:00" utc)
    peak_end_ts=$(epoch_at "${today_utc} 19:00:00" utc)
    dow=$(date -u +%u)
fi
